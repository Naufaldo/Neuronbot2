...
class WaterfillNavigation:
    ...
    def navigate(self):
        ...
        wall_following_direction = self.perform_wall_following(self.laser_data)

        # Combine directions (you may need to adjust this part based on your strategy)
        combined_direction = self.combine_directions(direction, obstacle_avoidance_direction, wall_following_direction)

        # Publish velocity command based on the combined direction
        cmd_vel = Twist()
        cmd_vel.linear.x = 0.1  # Example linear velocity
        cmd_vel.angular.z = combined_direction  # Example angular velocity
    ...
    def perform_wall_following(self, laser_data):
        # Perform wall following using laser scan data
        # Example: Follow the wall on the left side
        # (You may need to adjust this based on your robot's characteristics and the environment)
        # Extract laser scan data corresponding to the left side of the robot
        left_scan_data = laser_data.ranges[len(laser_data.ranges)//2:]

        # Find the minimum distance from the left side
        min_distance_left = min(left_scan_data)

        # If the minimum distance is greater than a threshold, continue forward
        if min_distance_left > 0.5:
            return 0

        # Otherwise, turn right to follow the wall
        return -np.pi / 4  # 45 degrees (right turn)

    def combine_directions(self, direction, obstacle_avoidance_direction, wall_following_direction):
        # Combine navigation direction with obstacle avoidance and wall following directions
        # Example: prioritize obstacle avoidance, then wall following
        if obstacle_avoidance_direction != 0:
            return obstacle_avoidance_direction
        elif wall_following_direction != 0:
